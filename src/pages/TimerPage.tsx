...
export default function TimerPage() {
  ...
  return (
    <div className="page-container">
      <div className="tabs">
        <button className={`tab ${currentMode === 'study' ? 'active' : ''}`} onClick={() => handleTabSelect('study')}>
          学习计时
        </button>
        <button className={`tab ${currentMode === 'shortBreak' ? 'active' : ''}`} onClick={() => handleTabSelect('shortBreak')}>
          短休息
        </button>
        <button className={`tab ${currentMode === 'longBreak' ? 'active' : ''}`} onClick={() => handleTabSelect('longBreak')}>
          长休息
        </button>
      </div>
